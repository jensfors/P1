...
cmp *source = malloc(MAX_LINE), *target = malloc(MAX_LINE);
char *ssource, *ttarget;
double simil = 0, avg_similar = 0, word_count = 0;
int source_word_count=0, target_word_count=0, word_lev=0, k, j, lev_dist=0;
  ...
  compare_struct_words(source, &source_word_count, target &target_word_count, &word_lev, &simil);
...
void compare_struct_words(cmp source, int source_word_count, cmp target, int target_word_count, int *word_lev, int *simil){
...
void is_simil(twitchchat offlinechat[], int simil, int word_count, int msg_nr){
double avg_similar=0;
  avg_similar = (simil/word_count);
  if(avg_similar > MIN_SIMIL){
    offlinechat[msg_nr].ref = chat[msg_nr-1].ref;
