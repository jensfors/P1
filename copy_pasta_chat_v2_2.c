...
double calc_similar(double avg_lev, double word_count); /* length_target_word insteadof target_word_count*/
  ...
  int numberoflines = 0, k = 0;
    ...
    if(chatfile != NULL){
        ...
        while(1){
            ...
            else if(chat_line[k].print == 0){
                printf("Msg nr: %d Ref: %d %s, Print: %d\n", 
                   chat_line[k].msg_nr,
                   chat_line[k].ref,
                   chat_line[k].username,
                   chat_line[k].print);
...
void get_twitch_chat(FILE *chatfile, twitchchat chat[]){
  static int i = 0;
  int source_word_count=0, target_word_count=0, word_lev=0, k, j, lev_dist, length_target_string=0;
  static double similar=0;
  double length_target_word, word_count = 0;
  
    
  char line[500], dummystr[500];
...
/*Add this*/                length_target_string = strlen(ttarget);
            string_to_words(source, target, ssource, ttarget, &source_word_count, &target_word_count);
/*Add this*/                length_target_word = length_target_string - target_word_count;
            ...
            for(k=0; k<source_word_count; k++){
                for(j=0; j<target_word_count; j++){
                    if(strncmp(target[j].word, source[k].word, 1)==0){
                        word_lev += ComputeLevenshteinDistance(source[k].word, strlen(source[k].word), target[j].word, strlen(target[j].word));
                        word_count += 1;
                        /*lev_dist = ComputeLevenshteinDistance(source[k].word, strlen(source[k].word), target[j].word, strlen(target[j].word)); */       
                        /*printf("%s\n%s\n Lev dist: %d, word_lev: %d\nTarget_word_count: %d\n", source[k].word, target[j].word, lev_dist, word_lev, target_word_count);
                        */
                    }
                }
            ...
            if(k == source_word_count){
                word_count = word_count +(length_target_string - word_count); 
                /*printf("avg_lev %f\n Target_word_count: %d\n", avg_lev(word_lev, source_word_count), target_word_count);*/
                similar = calc_similar(avg_lev(word_lev, source_word_count), word_count); /* length_target_word insteadof target_word_count*/
                printf("Similar: %f\n\n", similar);
            }   
    ...
    printf("Ttarget: %s\n", ttarget);*/
        ...
        while(source_tok != NULL){
         ...
         if(strcmp(source[something].word, source_tok)==0){
            ...
            strcpy(source[j].word, source_tok);
            *source_word_count +=1;
            source_tok = strtok(NULL, " ");
            }
...
double avg_lev(int word_lev, int source_word_count){
return ((double)word_lev/(double)source_word_count);
}    


double calc_similar(double avg_lev, double word_count) { /* length_target_word insteadof target_word_count*/
return (1.0 - (avg_lev/word_count));
}
