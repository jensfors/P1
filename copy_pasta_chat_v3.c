...
double calc_similar(double avg_lev, double length_target_word); /* length_target_word insteadof target_word_count*/
...
void get_twitch_chat(FILE *chatfile, twitchchat chat[]){
  static int i = 0;
  int source_word_count=0, target_word_count=0, word_lev=0, k, j, lev_dist, length_target_string=0, m = 0, l;
  
  static double similar=0;
  double length_target_word;
    ...
    if(fgets(line, sizeof(line), chatfile) != NULL){
        ...
        if(i>0){
            select_source_target(chat[i].text, chat[i-1].text, ssource, ttarget);
                length_target_string = strlen(ttarget);
            string_to_words(source, target, ssource, ttarget, &source_word_count, &target_word_count);
                length_target_word = length_target_string - target_word_count;
            ...
            if(k == source_word_count){
                ...
                similar = calc_similar(avg_lev(word_lev, source_word_count), length_target_word); /* length_target_word insteadof target_word_count*/
                printf("Similar: %f\n", similar);
            }   
...
double calc_similar(double avg_lev, double length_target_word) { /* length_target_word insteadof target_word_count*/
return (1.0 - (length_target_word/avg_lev));
}
